...
/**
 * v66 floating torso assistant.
 *
 * The overlay has no card, border or opaque background. The local photoreal portrait is
 * clipped to a compact head/shoulder/torso silhouette. Blink, gaze, micro-expression,
 * mouth, jaw, neck and shoulder motion deform source pixels only.
 */
public final class FloatingAssistantViewV66 extends View {
    ...
    public FloatingAssistantViewV66(Context context) {
        super(context);
    ...
    private void updateFrame() {
        ...
        portrait.setSpeechEnergy(speechFace.getSpeechEnergy());
        portrait.update(decision, dt);

        float speech = speechFace.getSpeechEnergy();
        motionLevel += ((0.07f + speech * 0.32f) - motionLevel)
                * Math.min(1f, dt * 4.2f);
    }
    ...
    private void buildTorsoMask(float w, float h) {
        ...
        // Neck, shoulders and upper torso. It intentionally overlaps the head sub-path,
        // producing one borderless silhouette instead of a rectangular widget card.
